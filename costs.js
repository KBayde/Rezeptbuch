...
// Feste Farben je Zutat-Kategorie für Kategorie-Donut/-Liste (rein dekorativ,
// an der CookCook-Markenpalette orientiert, aber bewusst unterscheidbar).
...
export async function renderHouseholdCosts(container) {
  ...
  async function load() {
        ...
        // Noch nicht abgehakte/erledigte Posten dieser Woche fließen als "live"
        // schon in die Auswertung ein, bevor sie beim Entfernen dauerhaft in
        ...
        async function renderTrend(name) {
                  ...
                  try {
                              ...
                              trendBody.innerHTML = `<p class="form-error">Preisverlauf konnte nicht geladen werden: ${escapeHtml(
                                            err.message
                                          )}</p>`;
                  }
        }
                    ...
                    .map((p) => {
                                  ...
                                  return `
                                              <div class="cost-bar" title="${escapeHtml(dateLabel)}: ${formatPrice(p.price)} €">
                                                            <div class="cost-bar-track">
                                                                            <div class="cost-bar-fill" style="height:${Math.max(
                                                                                                  4,
                                                                                                  Math.round((p.price / max) * 100)
                                                                                                )}%"></div>
                                                                                                              </div>
                  ...
                  return `
                          <div class="trend-summary">
                                    <span class="trend-latest">${formatPrice(trend.latest)} €</span>
                                              <span class="text-muted">Ø ${formatPrice(trend.average)} € · ${trend.points.length} Preis${
                                                          trend.points.length === 1 ? "" : "e"
                                              }</span>
