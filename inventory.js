...
export async function renderInventory(container) {
  container.innerHTML = \`
    <div class="page-header">
      ...
      <div class="inventory-header-actions">
        <a href="#/vorrat/foto-import" class="btn btn-secondary">📷 Per Foto erfassen</a>
        <button type="button" id="inv-open-manual" class="btn btn-primary">✏️ Händisch erfassen</button>
      </div>
    </div>

    <div class="storage-picker" id="storage-picker"></div>
    ...
    <div class="modal-overlay" id="inv-add-modal" hidden>
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="inv-add-modal-title">
        <div class="modal-header">
          <h2 id="inv-add-modal-title">Vorrat händisch erfassen</h2>
          <button type="button" class="modal-close" id="inv-add-modal-close" aria-label="Schließen">✕</button>
        </div>
  ...
  const storagePicker = container.querySelector("#storage-picker");
  const addModal = container.querySelector("#inv-add-modal");
  const openManualBtn = container.querySelector("#inv-open-manual");
  const closeManualBtn = container.querySelector("#inv-add-modal-close");
  ...
  addModal.addEventListener("click", (e) => {
    if (e.target === addModal) closeManualModal();
  });
  ...
  function renderStoragePicker() {
    const counts = { kuehlschrank: 0, gefrierschrank: 0, vorrat: 0 };
    allItems.forEach((i) => {
    ...
    storagePicker.querySelectorAll(".storage-card").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        filterStorage = filterStorage === loc ? "" : loc;
        renderStoragePicker();
        applyFilters();
      });
    });
